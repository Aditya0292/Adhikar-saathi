...
import { Star, MapPin, Phone, Briefcase, ChevronDown, ChevronUp, Languages } from 'lucide-react';
...
export default function LawyerCard({ lawyer, showPhone = false, compact = false }: LawyerCardProps) {
  ...
  const langs = lawyer.languages || [];
  const name = lawyer.full_name || 'Advocate';
    ...
    <div className="bg-white rounded-2xl border border-black/8 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full overflow-hidden">
      <div className={`p-4 flex flex-col flex-1 ${compact ? '!p-3.5' : 'md:p-5'}`}>
        ...
        <div className="flex justify-between items-start mb-2.5">
          <div className="flex items-center gap-2.5">
            <div className={`rounded-full bg-nyaya-green/10 flex items-center justify-center flex-shrink-0 ${compact ? 'w-9 h-9' : 'w-12 h-12'}`}>
              <span className={`font-serif font-bold text-nyaya-green-mid ${compact ? 'text-sm' : 'text-xl'}`}>{name[0]}</span>
            </div>
            <div>
              <h3 className={`font-sans font-semibold text-nyaya-text-dark leading-tight ${compact ? 'text-sm' : 'text-base md:text-lg'}`}>{name}</h3>
        ...
        <div className={`text-xs md:text-sm text-nyaya-muted space-y-1 mb-2.5 ${compact ? (expanded ? 'block' : 'hidden') : (expanded ? 'block' : 'hidden md:block')}`}>
          ...
          {langs.length > 0 && (
            <div className="flex items-center gap-2">
              <Languages size={compact ? 12 : 14} className="text-nyaya-muted/70" />
              <span className="capitalize">{langs.join(', ')}</span>
            </div>
          )}
        ...
        {compact && !expanded && (
          <div className="text-[10px] text-nyaya-muted flex flex-wrap items-center gap-2 mb-2 font-sans font-medium">
            <span className="flex items-center gap-1">
              <Briefcase size={10} className="text-nyaya-muted/70" />
        ...
        <div className="mt-auto flex gap-2">
          {showPhone ? (
            <NyayaButton 
              ...
              size="sm" 
              fullWidth 
              className="gap-2 rounded-xl py-1.5 text-xs"
              onClick={(e) => {
                e.stopPropagation();
                // If phone is verified and accepted, show alert or proceed
              }}
            >
